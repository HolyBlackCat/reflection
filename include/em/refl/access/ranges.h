...
namespace em::Refl::Ranges
{
    ...
    // Our range concept.
    template <typename T>
    concept Type = Meta::cvref_unqualified<T> && std::ranges::forward_range<T>;

    // Same, but maybe with cvref-qualifiers.
    template <typename T>
    ...
    template <typename T>
    concept ShouldForwardElements = TypeMaybeCvref<T> && _adl_em_refl_RangeShouldForwardElements(custom::AdlDummy{}, (const std::remove_cvref_t<T> *)nullptr);
    ...
    template <TypeMaybeCvref T, Meta::Deduce..., typename E>
