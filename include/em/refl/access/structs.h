...
namespace em::Refl::Structs
{
    namespace detail
    {
        ...
        // and fall back to `_adl_em_refl_StructFallback()` otherwise (that's primarily for internal user, but you user can provide it too).
        ...
        template <typename T>
        using Traits = typename SelectTraits<std::remove_cvref_t<T>>::template type<std::remove_cvref_t<T>>;
    }


    // A struct type. Must either contain `EM_REFL` macro or implement the same customization points.
    template <typename T>
    concept Type = requires{typename detail::Traits<T>;};
    ...
    template <typename T>
    concept HasMemberNames = Type<T> && requires{detail::Traits<T>::GetMemberName(0);};

    template <HasMemberNames T>
    [[nodiscard]] constexpr std::string_view GetMemberName(int i)
    {
        return detail::Traits<T>::GetMemberName(i);
    }
